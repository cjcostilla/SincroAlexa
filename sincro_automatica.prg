...
FUNCTION Sincro_Automatica_Dbf_Sql(cTablaDir)
	...
	* Consultar la tabla de auditoría para obtener los cambios pendientes
	...
	SCAN
			...
			CASE cAccion == "INSERT"
				* Crear listas de campos y valores mapeados
				...
				FOR i = 1 TO ALEN(aCampos,1)
					...
					IF VARTYPE(cValorMySQL) <> "N" AND VARTYPE(cValorMySQL) <> "D" AND VARTYPE(cValorMySQL) <> "T"
						IF (cValorMySQL = "TRUE" OR cValorMySQL = "FALSE")
							cValorMySQL =  IIF(cValorMySQL="TRUE",1,0)
						ELSE
							IF ATC('CTOD',cValorMySQL) = 0 AND ATC('CTOT',cValorMySQL) = 0
		...
		* Ejecutar la consulta y abrir el cursor
		IF NOT odb.QUERY(cEjecutar, curTablaSQL, cTablaMySQL)
			MESSAGEBOX("Error al ejecutar la consulta: " + cEjecutar)
			RETURN .F.
		ENDIF
		* Verificar si el cursor es editable
		IF NOT odb.CursorEdit(curTablaSQL)
			MESSAGEBOX("No se puede editar el cursor: " + curTablaSQL)
			RETURN .F.
		ENDIF
		...
		* Verificar los cambios en el cursor
		IF NOT odb.CursorChanges(curTablaSQL)
			MESSAGEBOX("Error al verificar cambios en el cursor: " + curTablaSQL)
			RETURN .F.
		ENDIF
		* Intentar actualizar y confirmar cambios
	...
	* Buscar la posición de la clave en la cadena JSON
	...
	DO CASE
		...
		CASE ContainsDate(lcValor)
			IF ATC(':',lcValor,1) > 0 AND ATC(':',lcValor,2) > 0
				lcValor = "CTOT('" + lcValor+ "')"
			ELSE
...
FUNCTION ParseJson(tcJSON,aCampos)
	LOCAL lcKey, lcValue, lcJson, lnPos, lnColonPos, lnCommaPos, lcMiValor
	...
	* Limpiar el JSON (quitar llaves y espacios)
	...
	DO WHILE !EMPTY(lcJson)
		...
		* Almacenar en el array
		...
		DO CASE
			CASE ISNUMERIC(lcValue)
				lcValue = VAL(lcValue)
			CASE ContainsDate(lcValue)
				IF ATC(':',lcValue,1) > 0 AND ATC(':',lcValue,2) > 0
					lcValue = "CTOT('" + lcValue + "')"
				ELSE
					lcValue = "CTOD('" + lcValue + "')"
				ENDIF
			CASE ContainsDateTime(lcValue)
				IF ATC(':',lcValue,1) > 0 AND ATC(':',lcValue,2) > 0
					lcValue = "CTOT('" + lcValue + "')"
				ENDIF
...
FUNCTION ObtenerMapeo(tcTablaVFP, tcCampoVFP, tcTipo, lbEsClave)
	...
	* Buscar el nombre mapeado en la tabla de mapeo
	SELECT tabla_mysql, campo_mysql, clave;
		FROM mapeo ;
		WHERE UPPER(tabla_vfp) = tcTablaVFP AND ALLTRIM(UPPER(campo_vfp)) == tcCampoVFP ;
					...
					CASE lcTipoDato = "C"  && Carácter
						...
						ELSE
							IF lcMappedName == "''" OR lcMappedName == '""'
								lcMappedName = "''"
							ELSE
								IF ATC('CTOD',lcMappedName) = 0 AND ATC('CTOT',lcMappedName) = 0
						...
						IF lcTipoDato <> "L"
							IF ATC('/  /',lcMappedName) > 0 AND EMPTY(lcResultadoFuncion)
								lcCampoPer = lcCampoPer + ;
									RTRIM(cursorMap.campo_mysql) + ' = ' + "CTOD("+lcMappedName+")" + ","
							ELSE
								lcCampoPer = lcCampoPer + ;
	...
	IF _ofiltro = 0
		DO CASE
			...
			CASE ALLTRIM(UPPER(_otablaSQL_funcion)) == 	ALLTRIM(UPPER("ABM_depositos"))
				...
				cConsulta = "SELECT ALLTRIM(UPPER(nombre)) AS NOMBRE FROM depósitos WHERE num = " + ALLTRIM(STR(_depositosq));
					+" INTO CURSOR cNomDep"
				&cConsulta
				SELECT cNomDep
				IF RECCOUNT() > 0
...
FUNCTION SQL_oDb_funcion_COD(otabla, onombre, ofiltro)
